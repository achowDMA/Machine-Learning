...
def neural_net_model(input_size):
    ...
    network = tf.fully_connected(network, 1000, activation='relu', name="hlayer1")
    network = tf.dropout(network, 0.8)

    network = tf.fully_connected(network, 1000, activation='relu' , name="hlayer2")
    network = tf.dropout(network, 0.8)

    network = tf.fully_connected(network, 1000, activation='relu', name="hlayer3")
    network = tf.dropout(network, 0.8)

    network = tf.fully_connected(network, 1000, activation='relu', name="hlayer4")
    network = tf.dropout(network, 0.8)
